...
static void test_basic_request(sd_event *e, const struct in_addr *start_address) {
        ...
        assert_se(sd_ipv4ll_new(&ll) == 0);
        assert_se(sd_ipv4ll_start(ll, start_address) == -EINVAL);
        ...
        assert_se(sd_ipv4ll_set_ifindex(ll, 1) == 0);
        assert_se(sd_ipv4ll_start(ll, start_address) == 1);
...
int main(int argc, char *argv[]) {
        struct in_addr start_address = { 0x0001FEA9 };
